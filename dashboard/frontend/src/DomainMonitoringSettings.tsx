...
function DomainRegistry({ active, onChanged, onAdminExpired }: {
  ...
  return <div className="domain-registry">
    ...
    <div className="domain-registry-heading"><h3>{t("Registrierte Domains")}{items !== null && ` (${formatNumber(items.length)})`}</h3>
    ...
    <div className="domain-monitoring-list">
      {items?.map((item) => <DomainRow key={item.domain} item={item} loading={loading}
        update={(patch) => mutate(item.domain, () => api.updateMonitoredDomain(patch))} />)}
    </div>
...
function DomainRow({ item, loading, update }: {
  item: DomainMonitoringItem;
  loading: boolean;
  update: (patch: DomainMonitoringPatch) => Promise<DomainMonitoringItem | null>;
}) {
  const { t, formatDate } = useI18n();
  const [grace, setGrace] = useState(String(item.grace_days));
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState("");
  const [message, setMessage] = useState("");
  const previousGrace = useRef(item.grace_days);
  const editVersion = useRef(0);
  const submittedVersion = useRef<number | null>(null);
  useEffect(() => {
    const previous = previousGrace.current;
    previousGrace.current = item.grace_days;
    const preserveNewerEdit = submittedVersion.current !== null && submittedVersion.current !== editVersion.current;
    if (!preserveNewerEdit) setGrace((current) => current === String(previous) ? String(item.grace_days) : current);
    submittedVersion.current = null;
  }, [item.grace_days]);
  const parsedGrace = graceValue(grace);
  const retired = item.state === "retired";
  const run = async (action: "grace" | "retire" | "reactivate") => {
    ...
    try {
      ...
      if (saved) setMessage(action === "grace"
      ...
      submittedVersion.current = null;
      setError(reason instanceof Error ? t(errorSources[reason.message] ?? reason.message) : t("Aktualisierung fehlgeschlagen."));
  ...
  return <article className="domain-monitoring-card" aria-label={item.domain} aria-busy={busy}>
    ...
    <form className="domain-monitoring-row-form" onSubmit={(event) => { event.preventDefault(); void run(retired ? "reactivate" : "grace"); }}>
      <label><span>{t("Wartefrist (Tage)")}</span><input type="number" required min={1} max={365} step={1}
        value={grace} onChange={(event) => { editVersion.current += 1; setGrace(event.target.value); }} aria-label={t("Wartefrist für {domain}", { domain: item.domain })} /></label>
      <div className="domain-monitoring-row-actions">
        ...
        {!retired && <button className="button button-ghost" type="button" onClick={() => void run("retire")} disabled={busy || loading}>{t("Stilllegen")}</button>}
      </div>
    </form>
